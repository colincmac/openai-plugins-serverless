...
const setupSignalRConnectionToChatHub = () => {
    const factory = () => AuthHelper.getSignalRAccessToken(AuthHelper.msalInstance, InteractionStatus.None);
    const connectionHubUrl = new URL('/api', process.env.REACT_APP_BACKEND_URI);
    ...
    return (next: Dispatch) => (action: SignalRAction) => {
        ...
        switch (action.type) {
            case 'conversations/updateConversationFromUser':
                hubConnection
                    .invoke('SendMessage', getSelectedChatID(), action.payload.message)
            ...
            case 'conversations/setConversations':
                Promise.all(
                    Object.keys(action.payload).map(async (id) => {
                        await hubConnection.invoke('AddClientToGroup', id);
                    }),
                ).catch((err) => store.dispatch(addAlert({ message: String(err), type: AlertType.Error })));
                break;
            case 'conversations/addConversation':
                hubConnection
                    .invoke('AddClientToGroup', action.payload.id)
                    .catch((err) => store.dispatch(addAlert({ message: String(err), type: AlertType.Error })));
                break;
        }
