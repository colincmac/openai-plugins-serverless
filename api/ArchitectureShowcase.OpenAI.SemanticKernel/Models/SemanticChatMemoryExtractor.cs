...
internal static class SemanticChatMemoryExtractor
{
		...
		PromptsOptions options)
	{
		...
		var remainingToken =
			...
			PluginUtilities.TokenCount(memoryPrompt);


		var memoryExtractionContext = PluginUtilities.CopyContextWithVariablesClone(context);
		...
		var memory = SemanticChatMemory.FromJson(result.ToString());
		return memory;
	}
