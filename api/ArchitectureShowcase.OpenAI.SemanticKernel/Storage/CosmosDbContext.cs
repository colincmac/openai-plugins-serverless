...
/// </summary>
public class CosmosDbContext<T> : IStorageContext<T>, IDisposable where T : IStoredEntity
{
	/// <summary>
	/// The CosmosDB client.
	/// </summary>
	...
	public async Task<T> ReadAsync(string entityId)
	{
		...
		try
		{
			var response = await _container.ReadItemAsync<T>(entityId, new PartitionKey(entityId));
